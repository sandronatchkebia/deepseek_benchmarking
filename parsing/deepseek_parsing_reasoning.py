...
def parse_file(infile: Path, dataset_name: str, model_name: str):
    ...
    wide_rows = []
    max_token_hits = 0  # Track queries that hit token limit
    
    for e in entries:
        # Check if this entry hit the token limit
    ...
    print(f"   Successfully completed: {len(entries) - max_token_hits}")
    if max_token_hits > 0:
        print(f"   ⚠️  {max_token_hits} queries may have incomplete responses due to token limits")
